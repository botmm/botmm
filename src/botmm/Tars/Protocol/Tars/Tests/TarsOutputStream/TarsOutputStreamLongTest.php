...
class TarsOutputStreamLongTest extends TarsTestCase
{
     ...
     * @param $data
     */
    public function testWriteLong($expected, $data)
    {
        $stream = new TarsOutputStream();
        $stream->writeLong($data, 1);
     ...
     * @param $byteArray
     */
    public function testWriteLongArray($expected, $byteArray)
    {
        $stream = new TarsOutputStream();
        $stream->writeLongArray($byteArray, 1);
        $this->assertEquals(hex2bin($expected), $stream->getByteBuffer());
    }
